...
> - **cilium-cli v0.19.x regression — `cilium clustermesh connect` writes the LOCAL clustermesh-apiserver Service DNS into the per-peer secret instead of the remote LB.** Caught during 2026-05-04 e2e v2: agent logs show `Connection to remote cluster established ... endpoints=[https://clustermesh-apiserver.kube-system.svc:2379]` for *every* peer, so all "remote" connections loop back to the local apiserver, mesh status sticks at `2/2 configured, 0/2 connected` indefinitely, and pod-to-pod cross-cluster traffic fails. Same `connect-mesh.sh` worked in v1 with cilium-cli v0.16.x. **Workaround:** pin cilium-cli to v0.16.x in your local env until the v0.19.x regression is identified and fixed upstream — the in-cluster Cilium chart version (1.19.3) is unaffected, only the CLI's secret-writing path is broken. Manual repair via `kubectl edit secret cilium-clustermesh` is theoretically possible but requires reconstructing each peer's TLS material (CA + client cert + key) from the remote cluster — not worth it; downgrade the CLI instead.
...
- [`cilium` CLI](https://github.com/cilium/cilium-cli) **v0.16.x** (`cilium version`). **Do NOT use v0.19.x** — it has a regression where `cilium clustermesh connect` writes the LOCAL clustermesh-apiserver Service DNS into the per-peer config secret instead of the remote LB, breaking all cross-cluster traffic. See the unresolved-issues callout above.
